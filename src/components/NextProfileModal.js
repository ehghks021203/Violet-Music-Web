...
import { ReactComponent as CloseIcon } from "/var/www/html/music-app/src/assets/imgs/BigClose.svg";
import { ReactComponent as CameraIcon } from "/var/www/html/music-app/src/assets/imgs/camera2.svg";
...
function NextProfileModal({ className, onClose, visible }) {
  ...
  const handleImageUpload = (file) => {
    ...
    if (isLoggedIn) {
      ...
      axios.post("https://blue.kku.ac.kr/userImg.php", formData, {
        headers: {
  ...
  const onChangeNicknameCheck = () => {
    ...
    axios.post("https://blue.kku.ac.kr/nicknameCheck.php",{
        "nickname" : nickname
    })
    .then(function (response) {
  ...
  const changeNickname = () => {
    ...
    if (confirmed) {
        axios.post("https://blue.kku.ac.kr/nicknameChange.php", {
  ...
  const withDrawal = ()=>{
    ...
    if(result){
    axios.post("https://blue.kku.ac.kr/withdraw.php", {
          "email": userData.email,
    })
    .then(function (response) {
      console.log(JSON.stringify(response));
  ...
  if (!isPasswordValid) {
    ...
    if (confirmed) {
      axios.post("https://blue.kku.ac.kr/passwordChange.php", {
          "email": userData.email,
          "password": password,
    })
    .then(function (response) {
